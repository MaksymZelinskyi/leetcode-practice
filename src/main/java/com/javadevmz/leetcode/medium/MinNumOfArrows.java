...
public class MinNumOfArrows {
    ...
    public int findMinArrowShots(int[][] points) {
        Arrays.sort(points, Comparator.comparingInt(x -> x[0])); //sort by the first element
        int count = 1;
        int last = points[0][1]; //track the last shot position
        for(int i = 0; i<points.length; i++) {
            if(points[i][0]>last) { //doesn't overlap
                count++; //one more arrow
                last = points[i][1];
            }else {
                last = Math.min(last, points[i][1]); //reassign to burst both balloons
            }
        }
        return count;
    }
